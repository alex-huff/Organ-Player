...
in_port = mido.open_input("MPK mini 3:MPK mini 3 MIDI 1 36:0")
mc_host = "localhost"
mc_port = 18589
...
def midi_note_to_create_note(note):
    return (
        clamp(midi_note_to_create_octave(note), 0, 2),
        midi_note_to_create_pitch(note),
    )


def send_update(is_pressed, octave, note, socket):
    if is_pressed == note_state[octave][note]:
        return
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
    ...
    for message in messages:
        ...
        if (
            ...
            and message.type != "note_off"
            and (message.type != "control_change" or message.control != 64)
        ):
            continue
